...
> **요약**: 서버실 출입관리 웹 시스템 개발 완료. Spring Boot 2.7 + MyBatis + PostgreSQL 기반 구현. 암호화 기능 적용, 메일 자동 발송, XSS 방어 등 3건 개선 조치 완료. 품질점수 62/100 → 71/100. 이후 관리 기능(출입 비밀번호 변경·공지사항·설정 패널)을 추가하고 미사용 의존성을 정리함(→ **7장 참조**). 운영 배포 시 Critical 3건(인증, 비밀번호 분리, DB 비밀번호 외부화) 적용 권장.
>
> **작성자**: 장석원  
> **작성일**: 2026-06-24 (갱신: 2026-06-24 추가 개발 반영)  
> **상태**: ✅ 완료
...
## 7. 추가 개발 내역 (2026-06-24 갱신)

초기 완료 보고 이후, 관리자 운영 편의를 위한 기능을 추가하고 미사용 의존성을 정리했다.

### 7.1 출입 비밀번호 변경 (관리자)

| 항목 | 내용 |
|------|------|
| **기능** | 관리자가 서버실 출입 비밀번호를 화면에서 변경 (고정값 → 변경 가능) |
| **저장** | `sram_config` 테이블(key/value)에 저장 — **재기동해도 유지** |
| **파일** | `ConfigMapper.java/.xml`(신규), `AccessLogService`(getAccessPassword/updateAccessPassword), `AccessApiController`(GET·POST `/api/access/password`), `admin.jsp`/`admin.js` |
| **반영** | 변경 즉시 이후 출입 신청 완료 모달에 새 비밀번호 표시 |

- 기존 하드코딩 상수 `7325*` → DB 설정값(기본값 `7325*` 폴백) 방식으로 전환
- 코드리뷰 S2(하드코딩) 일부 완화 — 단, 무인증 노출(S1·S2)은 여전히 운영 전 보완 필요

### 7.2 비밀번호 마지막 변경일 표시

- `sram_config.updated_at` 기준으로 **"마지막 변경: yyyy-MM-dd (N일 전)"** 표시
- API `GET /api/access/password` 응답에 `updatedAt`·`daysSince` 포함

### 7.3 공지사항 (작성/저장/삭제)

| 항목 | 내용 |
|------|------|
| **기능** | 관리자가 공지 작성·저장·삭제, **사용자 화면 상단 배너로 표시** |
| **숨김 처리** | 공지가 없으면(빈 값) 사용자 화면 배너 자동 숨김(`<c:if test="${not empty notice}">`) |
| **저장** | `sram_config`(key=`notice`), 최대 200자 |
| **파일** | `AccessLogService`(getNotice/updateNotice), `AccessApiController`(POST `/api/access/notice`), `PageController`(모델 전달), `user.jsp`(배너), `admin.jsp`/`admin.js` |
| **보안** | 배너 출력에 `<c:out>` 적용(XSS 방어) |

### 7.4 설정 접기 패널 (UI 정리)

- 비밀번호·공지 카드를 **"⚙ 설정" 패널 하나로 통합**, 기본 **접힘**
- 접힌 상태에서 대시보드(KPI·검색·출입 기록)가 먼저 보이도록 화면 정리
- `admin.jsp`(토글 구조), `admin.js`(토글 로직)

### 7.5 미사용 의존성 정리

- **Lombok 제거**: 코드 전반에서 Lombok 어노테이션 미사용 확인 후 `pom.xml`에서 제거 (DTO는 수동 getter/setter 사용)
- clean compile 정상 확인
...
| 객체 | 용도 |
|------|------|
| `sram_config` 테이블 | key/value 설정 저장 (출입 비밀번호, 공지사항). iBiz 충돌 방지를 위해 `sram_` 접두어 |

### 7.7 추가 검증

| 항목 | 결과 |
|------|------|
| 비밀번호 변경/조회/복원 | ✅ API E2E (7325* ↔ 변경값) |
| 변경일·경과일수 | ✅ `daysSince`·`updatedAt` 응답 확인 |
| 공지 저장 → 사용자 배너 노출 | ✅ |
| 공지 삭제(빈 값) → 배너 숨김 | ✅ (배너 표식 📢 미출력 확인) |
| 설정 패널 접기/펼치기 | ✅ 기본 접힘, 토글 동작 |
| Lombok 제거 후 빌드 | ✅ clean compile 성공 |
